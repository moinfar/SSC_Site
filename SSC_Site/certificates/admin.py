...
class CertificateAdmin(admin.ModelAdmin):
    ...
    list_display = (
    'name', 'data', 'template', 'compile_certificate', 'get_pdf_output', 'get_compile_process')
    ...
    def compile_certificate(self, obj):
        if obj.compile_process.status == 'R':
            ...
            return '<a href="%s">%s</a>' % (
            reverse('compile_certificate', args=(obj.id,)), _("Compile"))
    ...
    def get_compile_process(self, obj):
        if obj.compile_process:
            link = urlresolvers.reverse("admin:subprocess_manager_subprocess_change",
                                        args=[obj.compile_process.id])
